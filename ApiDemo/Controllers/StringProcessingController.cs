...
namespace Web_API_NET6.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class StringProcessingController : ControllerBase
    {
        ...
        public ActionResult<string> Manipulate(string input, string sortType)
        {
            if (input.All(c => c >= 'a' && c <= 'z'))
            {
                ...
                string[] blacklist = _config.GetSection("Blacklist").Get<string[]>();

                if (blacklist.Any(w => input.Contains(w)))
                {
                    string forbiddenWords = string.Join(", ", blacklist.Where(w => input.Contains(w)));
                    return BadRequest($"The input string contains forbidden words: {forbiddenWords}");
                }
        ...
        static int GetRandomIndex(int maxLength)
        {
            ...
            using (var client = new HttpClient())
            {
                try
                {
                    ...
                    var randomApi = MyConfig.GetValue<string>("RandomApi");
                    var response = client.GetAsync($"{randomApi}{maxLength - 1}&col=1&base=10&format=plain&rnd=new").Result;
